...
init_prompt = """Generate a Django application with Authentication, JWT, Tests, DB support. Show docker-compose for python and postgres. Show the complete code for every file!"""
prompts = []
prompt = ""
files = os.listdir('./prompts')
files.sort()
for prompt_file in files[:1]:
    filename=f'./prompts/{prompt_file}'
    print(filename)
    with open(filename) as f:
        text = f.read()
        print(len(text.split()))
        prompt += text
        #prompts.append(f'Context:\n{prompt}\n\nGiven the above context, answer the question: {init_prompt}')
...
class LLMUser(HttpUser):
    ...
    def _on_start(self):
        ...
        self.max_tokens_sampler = LengthSampler(
            ...
            mean=self.environment.parsed_options.max_tokens, # 200
        ...
        self.tokenizer = InitTracker.load_tokenizer(
            self.environment.parsed_options.tokenizer or self.environment.parsed_options.model
        )
        if self.tokenizer:
    ...
    def _get_prompt(self):
        ...
        res = prompt_template.format(cur_prompt=prompt)
        prompt_tokens = self.environment.parsed_options.prompt_tokens

        tokens = self.tokenizer.encode(res)
        diff = len(tokens) - prompt_tokens
        assert diff >=0
        res = self.tokenizer.decode(tokens[-prompt_tokens:])

        print(f"TOKENIZED LEGNTH: {len(self.tokenizer.encode(res))}")
        ...
        with self.client.post(
            ...
            for chunk in response.iter_lines(delimiter=b"\n"):
                ...
                try:
                    ...
                    if out.text and t_first_token is not None:
                        #dur_chunks.append(now - t_prev)
                        tbt_recorder.append(now - t_prev)
                        t_prev = now
                        #print(f"Next token received: {out.text}, in time {dur_chunks[-1]}")
                    elif out.text and t_first_token is None:
                        t_first_token = time.perf_counter()
                        print(f"First token received: {out.text}, in time {t_first_token - t_start}")
            ...
            if num_tokens:
                ...
                add_custom_metric(
                    "time_per_output_token", dur_generation / num_tokens * 1000, num_tokens
                )

                #dur_chunks.sort()
                #dur_len = len(dur_chunks)
                #dur_p99 = dur_chunks[-int(dur_len * 0.01)]
                #dur_p90 = dur_chunks[-int(dur_len * 0.1)]
                #dur_p50 = dur_chunks[-int(dur_len * 0.5)]

                #add_custom_metric(
                #    "time_between_tokens_p99", dur_p99 * 1000, 1
                #)
                #add_custom_metric(
                #    "time_between_tokens_p90", dur_p90 * 1000, 1
                #)
                #add_custom_metric(
                #    "time_between_tokens_p50", dur_p50 * 1000, 1
                #)
...
def _(environment, **kw):
    ...
    for metric_name in [
        ...
        "num_tokens",
        "total_latency",
        "prompt_tokens",  # might overwrite the static value based on server side tokenization environment.stats.entries["time_between_tokens_p99"].to_dict()
    ]:
